...
| **4** | Canvas editor: drag/resize/crop with snapping to the margin guides | **First step built 2026-09-16:** hand cropping. Drag a photo inside its slot to move the window, scroll to zoom; `BookEdit::SetCrop { placement, x, y, w }` re-derives the height from the slot's aspect (so a hand crop cannot distort), keeps the window inside the photo, and re-runs `score::rejects`. Live preview is local (`cropMoved`/`cropZoomed`, unit-tested), one edit is sent on release. A regenerate or swap of that placement recomputes the crop, documented in the README. **Not built:** moving or resizing the SLOT itself (`slot_rect`), snapping to the guides, adding or removing slots. Those change the page's geometry rather than the photo's window and want `preflight_core` run per edit plus `in_trim`/`in_safe_margin`/`clear_of_gutter` as snap targets. |
